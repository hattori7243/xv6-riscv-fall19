...
// 返回alloc的值
void alloc_not(char *array, int index)
{
  char b = array[index / 16];
  char m = (1 << ((index / 2)) % 8);
  if (alloc_isset(array, index))
    array[index / 16] = (b & ~m);
  else
    array[index / 16] = (b | m);
}
...
bd_malloc(uint64 nbytes)
{
  ...
  //不像之前分配一块则对那块的allooc置1，而是分配那块的取反
  alloc_not(bd_sizes[k].alloc, blk_index(k, p));
  for (; k > fk; k--)
  {
    ...
    alloc_not(bd_sizes[k - 1].alloc, blk_index(k - 1, p));
...
void bd_free(void *p)
{
  ...
  for (k = size(p); k < MAXSIZE; k++)
  {
    ...
    //等于1的话，说明只有自己这块被分配了，则可以合并
    //等于0的话，说明两块都被分配了，则跳出循环，free自己就可以了
    if (!(alloc_isset(bd_sizes[k].alloc, buddy)))
    { // is buddy allocated?
      alloc_not(bd_sizes[k].alloc, bi);
      break; // break out of loop
    }
    alloc_not(bd_sizes[k].alloc, bi); // free p at size k
...
int bd_initfree_pair(int k, int bi, int is_left)
{
  int buddy = (bi % 2 == 0) ? bi + 1 : bi - 1;
  int free = 0;
  if (alloc_isset(bd_sizes[k].alloc, bi))
  {
...
void bd_init(void *base, void *end)
{
  ...
  printf("free == BLK_SIZE(MAXSIZE) - meta - unavailable\n");
}
